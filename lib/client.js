...
class Client {
   ...
   * @resolves {{credentials:Object, params:Object}}
   * @private
   */
  _validateOptions(apiName, options) {
    const credentials = _.pick(options, CREDENTIAL_PROPERTIES);
    const params = _.omit(options, CREDENTIAL_PROPERTIES);


    return this._validateData('credential', credentials)
      .then(() => this._validateData(apiName, params))
      .then(() => {
        return Promise.resolve({credentials, params});
      })
  }
   ...
   * @returns {Promise}
   */
  setRequestOptions(apiName, credentials, params = {}, payload = {}) {
    this._host = this._sanitizeHost(credentials.host);
    this._url = this._getUrl(apiName, credentials.host, params);
    this._body = payload;
    this._method = apiList[apiName].method;
  ...
  getToken(options) {
    return this
      ...
      .then((response)=>{
        ...
        return this.setRequestOptions('getTimezone', credentials);
      })
      .then(()=>{
  ...
  getTimezone(options) {
    return this
      ._validateOptions('getTimezone', options)
      .then((response)=>{
        const {credentials} = response;
        return this.setRequestOptions('getTimezone', credentials);
      })
