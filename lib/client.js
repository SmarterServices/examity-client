...
class Client {
   ...
   * @param {string} options.clientID - The client ID to request
   * @param {string} options.secretKey - The secretKey against the client id
   * @param {string} options.userId - User id to get user Profile
   * @param {string} [options.accessToken] - The accessToken for api call
   * @returns {Promise<Object>} - Details of user
   */
  ...
  /**
   * Retrieve the examStatus for the given TransactionID
   * @param {Object} options - The option necessary for request
   * @param {string} options.host - The host name
   * @param {string} options.clientID - The client ID to request
   * @param {string} options.secretKey - The secretKey against the client id
   * @param {number} options.transactionID - TransID
   * @param {string} [options.accessToken] - The accessToken for api call
   * @returns {Promise<Object>} - Details of user
   */
  getExamStatus(options) {
    const apiName = 'getExamStatus';

    return this
      ._validateOptions(apiName, options)
      .then((response) => {
        const {credentials, payload} = response;
        const params = {transactionId: payload.transactionId};

        return this._sendRequest(apiName, credentials, params, payload);
      })
      .then((response) => {
        const session = _.get(response, 'appointmentStatusInfo.appointmentStatus.[0]', {});
        return session;
      });
  }
